...
A SuperVault can be deployed onchain before it is operationally live. Use this page to close the gap between **created** and **available**.
...
Upkeep is the activation gate. A vault can be created and visible to managers by direct link while still not active or discoverable. Fund upkeep from **Vault Settings → Upkeep**. The activation route can deep-link with `panel=upkeep&activateVault=1` so the primary manager lands on the funding action.

Current Erebor-facing activation fields used by the UI include:

| Field | Meaning |
|-------|---------|
| `activation.is_active` | Whether the vault has cleared activation requirements. |
| `activation.is_discoverable` | Whether the vault can appear in discovery surfaces. |
| `upkeep_info.balance` | Current upkeep balance. |
| `upkeep_info.payment_enabled` / `is_upkeep_payment_enabled` | Whether upkeep payment is enabled for the vault. |
| `upkeep_info.threshold.required_balance` | Required balance for activation. |
| `upkeep_info.threshold.shortfall` | Amount still needed to clear the gate. |
...
Do not route deposits into a vault just because creation succeeded. Wait for the checklist to show that upkeep, hooks, yield sources, strategies, and listing availability are all complete.
