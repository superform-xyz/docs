...
SuperVaults enforces four permission tiers with distinct onchain and offchain boundaries:

| Role | On-Chain? | Scope | Mutable? |
|------|-----------|-------|---------|
| **Primary Manager** | Yes | Full vault control | Immutable (set at creation) |
| **Secondary Manager** | Yes | All operations except vault settings | Removable via onchain tx |
| **View Only** | No | Dashboard read access | Added/removed offchain |
| **Registry Maintainer** | No | Global registry CRUD | DB-level grant |

## Primary Manager

The primary manager is the onchain vault owner. One per vault.
...
Off-chain read access. No onchain transaction required.
...
Global permission for administering Erebor registries. Not vault-scoped.
...
The institutional dashboard enforces permissions at both the page level and the API level:
...
# Add view-only user (offchain, no gas)
