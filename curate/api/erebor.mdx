...
<Note>
Settings changes (fees, PPS parameters, timelocks, banned leaves) are **onchain transactions**. The curator UI constructs calldata via Erebor's `/prepare` endpoints and your wallet signs and submits them to the protocol contracts directly. No Erebor REST endpoint is called for the actual setting change.
</Note>
...
| `/api/v1/users/view-only` | POST | Add view-only user (offchain) |
...
| `/api/v1/vaults/emergency-pause/prepare` | POST | Prepare onchain pause tx |
| `/api/v1/vaults/emergency-unpause/prepare` | POST | Prepare onchain unpause tx |
