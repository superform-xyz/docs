...
Curate is the institutional control plane for SuperVaults. It lets managers deploy vaults, configure launch readiness, authorize keeper actions, whitelist yield sources, run automated strategies, monitor execution, and intervene during incidents.

The core invariant: **a vault is not live just because it exists onchain.** A production vault needs upkeep, hook authorization, yield sources, strategy coverage, and catalog availability before users should rely on it.
