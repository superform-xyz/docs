...
<Steps>
  ...
  <Step title="JWT Stored in Browser">
    ...
    Tokens are also held in-memory via `tokenService` for injection into Erebor and OMS API clients.
  </Step>
  <Step title="Auth Verified (Erebor-first)">
    The app calls `GET /api/v1/auth/me` on Erebor as the **primary session gate**. If this succeeds, the session is considered authenticated. A secondary call to Persephone `/admin` enriches the `userType` field from the product catalog — this call is non-fatal; its failure does not break the session.
  </Step>
</Steps>
