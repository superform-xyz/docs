...
The **Home** page (`/`) is the top-level entry point at [curators.superform.xyz](https://curators.superform.xyz). It provides a global overview of all SuperVault deployments. No authentication required. Data refreshes from `GET /api/v1/public/vaults` (Erebor) with a 5-minute stale time.
...
Home is one of three top-level sections in the curator app:
