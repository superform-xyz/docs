...
description: "Emergency onchain vault pause and per-vault keeper service controls."
---
...
The Pause page provides two control mechanisms: onchain vault pause (halts contract-level execution, requires gas) and per-vault keeper service controls (halts individual workers, no gas). Both are fully operational and wired to Erebor.
...
Shows pause status for all vaults you curate. Each vault displays name, chain, pause state, timestamp, and actor.
...
When paused: Strategy Engine stops dispatching intents, all onchain vault operations halt, PPS oracle updates may be blocked.
...
Independent of the onchain pause. Control individual workers for a specific vault without gas.
