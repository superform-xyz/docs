...
<Note>
SuperVault contracts are live on-chain and fully non-custodial. The curator UI at [curators.superform.xyz](https://curators.superform.xyz) is a management layer — it never holds keys or assets. All vault interactions are direct on-chain transactions signed by your wallet.
</Note>
...
|-----------|-------------|
| **Current PPS** | Last on-chain recorded value |
...
A keeper can only call a hook if the exact combination of hook address + parameters matches a leaf in the vault's on-chain merkle tree.
...
When a keeper executes a hook, the on-chain contract verifies the merkle proof. If the proof is invalid (the hook + params combination was not authorized), the transaction reverts.
...
|--------|---------------|
| `fulfill_deposits` | Allocates free vault assets to yield sources per the deposit ladder |
...
- **Ladder configuration** (directs deposit and redemption behavior)

## The Strategy Engine

The Strategy Engine adds rule-based automation on top of the keeper system. Instead of static ladder ordering, curators define strategies with:
